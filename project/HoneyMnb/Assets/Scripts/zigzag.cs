...
public class zigzag : MonoBehaviour
{
    ...
    private void MoveVertices(Vector3 direction){
            foreach (int i in PositiveIndex)
            {
                if(checkDistance(positiveSideObject, i, true)){
                    ...
                    positiveVertices[i] += new Vector3(direction.y, -direction.x, 0) * moveSpeed;
                    positiveUVs[i] += new Vector2(direction.y, -direction.x) * moveSpeed;
            ...
            foreach (int i in NegativeIndex)
            {
                if(checkDistance(negativeSideObject, i, false)){
                    ...
                    negativeVertices[i] += new Vector3(-direction.y, direction.x, 0) * moveSpeed;
                    negativeUVs[i] += new Vector2(-direction.y, direction.x) * moveSpeed;
    ...
    private void RePosition(){
            ...
            positiveSideObject.transform.localScale = new Vector3(1,4,1);
            ...
            negativeSideObject.transform.localScale = new Vector3(1,4,1);
    }
    ...
    private void Switching(){
        ...
        SliceObject(sliceDirection, Vector3.zero);
